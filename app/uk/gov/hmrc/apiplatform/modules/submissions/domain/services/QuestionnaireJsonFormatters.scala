...
trait QuestionnaireJsonFormatters extends QuestionJsonFormatters with AskWhenJsonFormatters with NonEmptyListFormatters {
  ...
  implicit val jsonFormatQuestionItem: Format[QuestionItem] = Json.format[QuestionItem]

  implicit val jsonFormatquestionnaire: Format[Questionnaire] = Json.format[Questionnaire]
}
